// Source: https://github.com/sigerror/fast-vector
...
template <class T>
fast_vector<T>::fast_vector(size_type count) : m_size(count), m_capacity(count)
{
    m_data = reinterpret_cast<T*>(std::malloc(sizeof(T) * count));
    assert(m_data != nullptr && "Allocation failed");

    if (!std::is_trivial_v<T>)
    {
        construct_range(m_data, m_data + count);
    }
}
...
template <typename T>
void fast_vector<T>:: resize_with_zero_init(size_type count) {
    if (count > m_capacity) {
        reserve(count);
    }

    assert(std::is_trivial_v<T> && "resize_with_zero_init() is only supported for trivial types");

    if (count > m_size) {
        std::memset(m_data + m_size, 0, sizeof(T) * (count - m_size));
    }
